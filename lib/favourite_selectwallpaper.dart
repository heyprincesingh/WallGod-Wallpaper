...
class _selectWallpaperState extends State<selectWallpaper> {
  ...
  getFavouritewallUrl(originalimagesUrl) async {
    ...
    originaltemp?.remove(originalimagesUrl);

    SharedPreferences originalFavouriteUrlsprefs = await SharedPreferences.getInstance();
    originalFavouriteUrlsprefs.setStringList("originalFavouriteUrls", originaltemp!);
          ...
          itemBuilder: (BuildContext context,index){
            return Container(
                ...
                child: Container(
                  ...
                  child: Image.network(
                      "${widget.imagesUrl![index]}?auto=compress&cs=tinysrgb&dpr=2&h=650&w=940",
        ...
        children: [
              ...
              var actionsheet = CupertinoActionSheet(
                ...
                actions: [
                  CupertinoActionSheetAction(onPressed: ()async{
                    ...
                    String url = widget.imagesUrl?[downloadindex!];
                  ...
                  CupertinoActionSheetAction(onPressed: ()async{
                    Fluttertoast.showToast(msg: 'Downloading...',
                        toastLength: Toast.LENGTH_SHORT);
                    Navigator.of(context).pop();
                    String url = "${widget.imagesUrl?[downloadindex!]}?auto=compress&cs=tinysrgb&dpr=2&h=650&w=940";
                    final time = DateTime.now().toIso8601String().replaceAll('.', '-').replaceAll(':', '-');
                    String name = "WallGod-$time";
                    var status = await Permission.storage.request();
                    if(status.isGranted){
          ...
          SpeedDialChild(
            ...
            child: Tooltip(
                ...
                child: Icon(CupertinoIcons.heart_slash_fill,color: Colors.red,)),
            backgroundColor: Color(0xffd5d5d5),
            onTap: (){
              getFavouritewallUrl(widget.imagesUrl?[downloadindex!]);
            },
            // label: "Favourite"
          ),
        ],
  ...
  setwallLocation(int? downloadindex, BuildContext context, int setwallpaperAs) async {
